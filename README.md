...
| `room-inbody` | 검사·상담 공간 (인바디 · 신장계) | 소아 진료 |
...
**아직 어디에도 쓰지 않는 파일** — `gift-wrap-bojagi` (보자기로 포장한
공진단 선물 그림). 보관만 해 둔 것이라 `src/lib/img.ts` 에 자리를 만들지
않았고 어느 페이지에도 나오지 않습니다. 쓸 자리가 정해지면 img.ts 에
이름을 넣고 Photos 로 배치하면 됩니다. 보자기 포장이 추석 행사
(9/1~10/18) 한정 혜택이라면, 상시 페이지에 넣을 때 문구에 주의하세요.
...
`care-*` 사진의 모델은 원장의 동생이며 본인 동의를 받았습니다
(원장 확인, 2026-09).
