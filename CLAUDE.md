...
- **건강칼럼 글 추가**: `src/content/columns/*.md` — frontmatter는 `title`(글 제목, H1과 `<title>`에 함께 쓰임), `summary`(목록과 메타 설명, 80자 이내), `date`(`YYYY-MM-DD`), `author`, `category`, `related`(관련 페이지 링크 배열), `seoTitle`(선택). **제목이 27자를 넘으면 `seoTitle`에 짧은 제목을 적으세요** — 뒤에 `| 시호한의원`(8자)이 붙어 35자를 넘으면 구글이 자릅니다. `seoTitle`은 `<title>`에만 쓰이고 화면의 H1은 `title` 그대로입니다. 파일 이름이 주소가 되고 목록·sitemap에 자동 반영됨. BlogPosting 구조화 데이터도 자동 생성됨. **`date`는 실제로 올리는 날을 적으세요** — 미래 날짜를 적으면 구조화 데이터의 `datePublished`가 아직 오지 않은 날이 됩니다. 같은 날 여러 편을 올려 목록 순서를 정하고 싶으면 시각까지 적으세요(`'2026-09-23T15:40:00+09:00'`, 따옴표 필요). 날짜 표시는 빌드 서버와 무관하게 한국 시간으로 나갑니다(`src/lib/date.ts`). **⚠ 치료 전후 사진과 환자 후기(치료경험담)는 올리지 마세요 — 의료법 제56조 제2항 금지**
